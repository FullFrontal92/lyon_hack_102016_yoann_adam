...
function showSocks($conn){
    $userVoter = $_GET['user'];
    $sql = "SELECT * FROM Photo WHERE User!='".$userVoter."'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
...
function showWinner($conn){
    $sql = "SELECT User, Votecontest FROM Photo ORDER BY Votecontest DESC LIMIT 3";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            echo '<a>'.$row['User'].' '.$row['Votecontest'].' '.'</a>';
        }
    } else {
        echo "0 results";
    }
